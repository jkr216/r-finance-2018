---
title: "Fama French Shiny Choose Factors"
runtime: shiny
output:
...
asset_returns_daily <- eventReactive(input$go, {
  prices <- prices()
  asset_returns <- na.omit(Return.calculate(prices, method = "log"))
})
  

  portfolio_returns_daily <- eventReactive(input$go, {
      asset_returns_daily <- asset_returns_daily()
...
factors_address <- paste("http://mba.tuck.dartmouth.edu/pages/faculty/ken.french/ftp/", input$factor, "_CSV.zip", sep="" )

factors_csv_name <- paste(input$factor, ".csv", sep="")

download.file(factors_address, temp)
...
rolling_ff_tidy <- eventReactive(input$go, {
